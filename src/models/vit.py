...
class ViT(nn.Module):
    def __init__(self, in_c: int = 3, num_classes: int = 10, img_size: int = 32, patch: int = 8, dropout: float = 0., num_layers: int = 7, hidden: int = 384, mlp_hidden: int = 384*4, head: int = 8, is_cls_token: bool = True, activation: str = 'relu'):
        ...
        enc_list = [TransformerEncoder(
            hidden, mlp_hidden=mlp_hidden, dropout=dropout, head=head, activation=activation) for _ in range(num_layers)]
...
class TransformerEncoder(nn.Module):
    def __init__(self, feats: int, mlp_hidden: int, head: int = 8, dropout: float = 0., activation: str = 'relu'):
